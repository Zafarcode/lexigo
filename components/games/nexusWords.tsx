...
const NexusWords: React.FC = () => {
    const { isOver, results, setResults, setShuffledWords, setShuffledTranslations, setSelectedWord, setSelectedTranslation, selectedTranslation, selectedWord, setIsOver, shuffledTranslations, shuffledWords } = useWordGameStore();
    ...
    useEffect(() => {
        ...
        const wordsArray = selectedWords.map(wordObj => ({ word: wordObj.word, translation: wordObj.translation }));
        const translationsArray = selectedWords.map(wordObj => ({ word: wordObj.word, translation: wordObj.translation }));
            ...
            <Dialog open={isOver} onOpenChange={() => setIsOver(false)}>
                <DialogContent className='sm:max-w-[425px]'>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            <p>
                                You scored <span className='font-bold text-green-500'>{results.filter((result) => result.isCorrect).length}</span> out of <span className='dark:text-white text-black'>{results.length}</span> correct answers.
                            </p>
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter className=''>
                        <Link href={'/vocabulary'} onClick={() => setIsOver(false)} >Try Again</Link>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            <div className="mt-8">
