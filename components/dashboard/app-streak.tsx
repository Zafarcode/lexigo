...
import { FireIcon, HeartIcon, StarIcon } from '@heroicons/react/24/solid'
...
const AppStreak = () => {
	...
	return (
		<div className='flex flex-col items-center dark:bg-zinc-950 right-0 fixed space-y-6 transition-all duration-300 pr-4 ease-in-out overflow-hidden w-full max-w-[450px]'>
			{/* Header Section */}
			<div className='flex items-center justify-between w-full pt-4 max-w-[450px] relative'>
				...
				>
					<button // This is the main button for the flag
				...
				{/* Fire Icon */}
				<button // This is the main button for the fire icon
				...
				{/* Star Icon */}
				<button // This is the main button for the star icon
				...
				{/* Lives Icon - Corrected Structure */}
				<div className='flex justify-center'>
					<div // Changed from <button> to <div> as this acts as a hover trigger/container
						className='relative group'
						onMouseEnter={() => setShowLiveCard(true)}
						onMouseLeave={() => setShowLiveCard(false)}
					>
						{/* This is the actual clickable heart icon button */}
						<button
							className='flex items-center gap-1 w-14 h-10 rounded-lg transition-all duration-300 group-hover:bg-gray-100 dark:group-hover:bg-gray-700'
							aria-label='Show Lives Information' // Added aria-label for accessibility
						>
								...
								<CardContent className='p-4 rounded-xl bg-gray-100 dark:bg-zinc-900 dark:border-b-white'>
									...
									<div className='flex justify-center'>
										{Array(5)
											.fill(0)
											.map((_, i) => (
												<HeartIcon key={i} className='w-7 h-7 text-pink-600' />
											))}
									</div>
